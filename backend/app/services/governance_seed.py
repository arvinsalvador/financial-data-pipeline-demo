...
ROLE_PERMISSIONS["finance_analyst"] = ROLE_PERMISSIONS["finance_analyst"] + tuple(
    code
    for code in EXCEPTION_PERMISSIONS
    if code not in {"exceptions.ignore_critical", "exceptions.close", "exceptions.view_sensitive"}
)
ROLE_PERMISSIONS["client_viewer"] = ROLE_PERMISSIONS["client_viewer"] + (
    "exceptions.view",
    "exceptions.statistics_view",
)
...
WORKFLOW_RULES = (
    ...
    (
        "control_total_high_priority",
        "Control Total High Priority",
        "control_totals",
        None,
        None,
        "high",
    ...
    (
        "duplicate_normal_priority",
        "Duplicate Normal Priority",
        None,
        "duplicate",
        None,
        "normal",
    ...
    (
        "resolved_redetection_reopens",
        "Resolved Redetection Reopens",
        None,
        None,
        None,
        "normal",
    ...
    (
        "ignored_redetection_remains_ignored_with_occurrence_increment",
        "Ignored Redetection Remains Ignored",
        None,
        None,
        None,
        "normal",
    ...
    (
        "pipeline_failure_assign_platform_admin",
        "Pipeline Failure Assign Platform Admin",
        "pipeline",
        None,
        None,
        "urgent",
    ...
    (
        "reconciliation_exception_assign_finance_analyst",
        "Reconciliation Exception Assign Finance Analyst",
        None,
        "reconciliation",
        None,
        "high",
...
RESOLUTION_CODES = (
    ...
    "accepted_valid_variance",
    "accepted_timing_difference",
    "duplicate_record_confirmed",
    "reversal_confirmed",
    "mapping_updated",
    "source_record_excluded",
    "manual_match_confirmed",
    "customer_credit_confirmed",
    "unapplied_payment_confirmed",
    "payroll_difference_confirmed",
    "false_positive",
    "expected_test_defect",
    "no_action_required",
    "deferred",
    "other",
)
...
def seed_governance_data(session: Session, settings: Settings) -> dict[str, int]:
    ...
    for _index, (
        ...
        order,
        reopen,
    ) in enumerate(WORKFLOW_RULES, 1):
        if (
            session.scalar(
                select(ExceptionWorkflowRule).where(
                    ExceptionWorkflowRule.tenant_id == tenant.id,
                    ExceptionWorkflowRule.code == code,
                    ExceptionWorkflowRule.version == "1.0.0",
                )
            )
            is None
        ):
            session.add(
                ExceptionWorkflowRule(
                    ...
                    exception_code_pattern=pattern,
                    severity=severity,
                    initial_priority=priority,
                    initial_status=status,
                    auto_assign_team_code=team,
                    escalation_after_hours=24 if priority in {"high", "urgent"} else None,
                    resolution_requires_comment=True,
                    ignore_requires_comment=True,
                    reopen_on_redetection=reopen,
                    configuration_json={},
                    version="1.0.0",
    ...
    for code, name, severity, response, resolution, escalation in SLA_POLICIES:
        if (
            session.scalar(
                select(ExceptionServiceLevelPolicy).where(
                    ExceptionServiceLevelPolicy.tenant_id == tenant.id,
                    ExceptionServiceLevelPolicy.code == code,
                )
            )
            is None
        ):
    ...
    for code in RESOLUTION_CODES:
        if (
            session.scalar(
                select(ExceptionResolutionCode).where(
                    ExceptionResolutionCode.tenant_id == tenant.id,
                    ExceptionResolutionCode.code == code,
                )
            )
            is None
        ):
